...
//form  ,lean model,  SpiLoader.get[InvokerProvider].instance(config, invokerName, messageProducer, poolConfig, limitConfig)
...
/*  private val pool =
  ...
  def handleActivationMessage(msg: ActivationMessage)(implicit transid: TransactionId): Future[Unit] = {
    ...
    println("************invoke handleActivationMessage**********************")
      ...
      .flatMap(action => {
        action.toExecutableWhiskAction match {
          case Some(executable) =>
            msg.showMessage
            println(executable)
            executable.printDetail
            //println(executable.containerInitializer())
  ...
  def processActivationMessage(bytes: Array[Byte]): Future[Unit] = {
    Future(ActivationMessage.parse(new String(bytes, StandardCharsets.UTF_8)))
      ...
      .flatMap { msg =>
        ...
        // active-ack.
        println("************invoke processActivationMessage**********************")
